...
const { entityStore, deviceOverview, hydrateScope, refreshDeviceOverview } = useCareData()

const managedDevices = computed(() => entityStore.devices)
const overview = computed(() => deviceOverview.value)

onMounted(async () => {
  if (!managedDevices.value.length) {
    await entityStore.fetchDevices()
  }
  if (!overview.value) {
    await refreshDeviceOverview()
  }
  if (!entityStore.persons.length) {
    await hydrateScope()
  }
})
      ...
      <div class="card highlight">
        <div class="summary-head">
          ...
          <div>
            ...
            <h3>{{ overview?.totalDevices ?? managedDevices.length }} 台</h3>
